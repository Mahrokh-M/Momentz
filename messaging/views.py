...
def inbox(request):
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                ...
                AND m.message_id IN (
                    ...
                    FROM Messages
                    WHERE sender_id IN (%s, u.user_id) AND receiver_id IN (%s, u.user_id)
                    GROUP BY CASE 
        ...
        conversations = [conv for conv in conversations if conv.get("chat_partner_username")]
...
def send_message(request):
    try:
        ...
        if not content or not receiver_id:
            return JsonResponse({"status": "error", "message": "Content and receiver_id are required"}, status=400)
